%% ieee-r10htc_merged.tex
%% IEEE R10-HTC 2026 conference paper — merged version.
%% Merge = v2 corrected bibliography + reviewed-PDF body + outstanding reviewer fixes.
%% Editorial call-outs are LaTeX comments beginning with "% MERGE:" — they do not render.
%% Figures are referenced by path; supply the PNGs or comment out the \includegraphics lines to compile a text-only proof.
%%
%% Compile: pdflatex ieee-r10htc_merged.tex  (run twice for references)
...
% Allow \approx, ≥, ×, etc. in text via math where needed.
...
% MERGE: Title reframed to the ports / forward-looking framing (reviewed PDF). v2 kept the old descriptive title.
% For double-blind submission, replace the author block with "\author{Author names withheld for double-blind review}".

\author{
\IEEEauthorblockN{Daniel Phang}
\IEEEauthorblockA{ORCID 0009-0006-3785-4458\\
Affiliation --- TBD\\
email --- TBD}
\and
\IEEEauthorblockN{Tze-Houng Lee}
...
\vspace{2pt}
\footnotesize
\textsuperscript{\dag}Fathom's free tranche (accessed and verified 2025; a dated snapshot is pinned for review) is restricted to selected developing countries and excludes the major ASEAN megacities covered here. The comparison axis is \emph{openness and access, not accuracy}: Fathom is bare-earth (FABDEM/FABDEM+) and engineering-validated, and its terrain treatment directly addresses the DSM-driven bias we characterise in Sec.~\ref{sec:validation}; we claim open reproducibility for omitted cities, not terrain or skill parity. C\,=\,coastal, F\,=\,fluvial, P\,=\,pluvial.
\end{table}
% MERGE [C1]: Table restructured to 5 columns (PDF form) and footnote rewritten to add the openness-not-accuracy framing + FABDEM acknowledgment. Neither was in v2 or the PDF; both are reviewer requests.

\section{Decision Context: Why Screening Must Be Forward-Looking}
% MERGE [B2]: Entire section + Figs. 1--2 present in the reviewed PDF, ABSENT from v2. Added.
Infrastructure optimised for the twentieth-century climate now faces conditions outside its design envelope---roads buckling in unprecedented heat, drainage overwhelmed by rainfall beyond prior high-water marks~\cite{jpm_intuition}. The core reason is a property of shifting distributions, not of any single forecast: as a hazard distribution shifts warmer, the frequency of extremes grows much faster than the mean. Fig.~\ref{fig:distshift} makes this concrete with a schematic Gaussian shift---a mean shift of one standard deviation raises the probability of exceeding a ``$+2\sigma$'' threshold from 2.3\% to 15.9\%, roughly a 7$\times$ jump that turns a 44-year event into a 6-year event, even though the average moved only modestly~\cite{patel2024}. (The 2.3\%/15.9\% values are the normal-distribution tail beyond $2\sigma$ and $1\sigma$; the figure illustrates the mechanism, not a specific IDF curve.) A planner who screens flood risk by extrapolating the historical record therefore systematically under-prices the tail. The corrective is to screen on return periods and explicit climate scenarios---exactly the surface our atlas provides.

The decision relevance is sharpened by the long life of the assets at risk (Fig.~\ref{fig:assetlife}). A port entering planning today is completed in the mid-2030s and operates into the 2080s, by which point SSP-pathway divergence is wide: median ASEAN SLR by 2100 spans $+0.62$ to $+1.62$~m, with larger high-end ranges. Two further realities frame how the screen will be used. First, adaptation planning lags decarbonisation: among the largest global ports, 89\% (31/35) have mitigation plans but only 66\% (23/35) have adaptation plans~\cite{jpm_seachange}. Second, hard remediation is slow and costly---the U.S. Coastal Texas (``Ike Dike'') barrier is projected at \$57~billion over 20~years~\cite{jpm_seachange}---so a cheap, open, comparable screen is the rational first move before any such commitment. Global port adaptation to SLR alone is estimated at \$223--768~billion by 2050; deciding \emph{where} to spend it begins with screening.

\begin{figure}[t]
\centering
% \includegraphics[width=\columnwidth]{docs/paper/figures/ieee_fig_distshift.png}
...
\begin{figure}[t]
\centering
% \includegraphics[width=\columnwidth]{docs/paper/figures/ieee_fig_assetlife.png}
...
\label{fig:assetlife}
\end{figure}
% MERGE [C7]: $57B, $223--768B, 89/66\%, 12--25x figures trace to JPMorgan advisory notes \cite{jpm_seachange,jpm_intuition}. Trace to primary literature before submission.

\section{Open Multi-Hazard Pipeline}
% MERGE [B3]: Expanded from v2 to the PDF form.
...
\begin{figure}[t]
\centering
% \includegraphics[width=\columnwidth]{docs/paper/figures/ieee_fig1_pipeline.png}
...
% MERGE [B4]: Table II rebuilt with bathtub->inertial column; mitigation-delta retraction. v2 had 3,598 combined and "robust to absolute bias".
...
\textbf{Two reported bars, stated explicitly.} We distinguish two criteria that an earlier draft conflated. \emph{Significant discriminative skill} is the weaker condition that the TSS confidence interval excludes zero. \emph{PASS} is the stricter, pre-registered bar HR $\geq 0.70$ \emph{and} CRR $\geq 0.70$ \emph{and} TSS CI $>0$. All four cities meet the weaker condition; one (Kuala Lumpur) meets PASS (Table~\ref{tab:gate}). Because the gate is run in four cities, we note the family-wise context: Kuala Lumpur's PASS survives a Bonferroni-adjusted $\alpha = 0.0125$, and so does the pooled register. We also acknowledge the small registers (24/58/23/26 points): the lower CI bounds of 0.03--0.04 for Bangkok and Jakarta are statistically significant but fragile, and we present them as such rather than as robust.

\textbf{Order of operations (no retuning).} Two design choices materially affect the gate, so we state their timing explicitly. (i)~The main-stem-HAND trunk threshold ($\geq 180$~km\textsuperscript{2} for Kuala Lumpur) was fixed by the physics of which channel the modelled discharge represents; it corrects a false positive on a 60--77~m hill and yields a credible floodplain. (ii)~The Jakarta control reclassification was decided on the 2007/2013 flood record. Both were fixed before the first gate evaluation and anchored to documented facts, not gate output; a genuine control that the model floods stays in the register as a reported false positive. The remaining shortfalls are \emph{documented structural limits, not tuning failures}: Bangkok's HR is bounded because the 2011 reference flood was sourced from a catchment whose headwaters lie outside the model domain~\cite{trinh2017}, and Jakarta's residual specificity loss is fill-and-spill over-ponding on genuinely elevated ground.

\textbf{Bathtub bias is corrected, not merely cheap.} A bathtub solver---the default in open screening tools---over-predicts documented present-day coastal inundation by 1.7--25$\times$ at RP100, because 30~m terrain cannot resolve the sub-pixel road raises, canals, and bunds that protect small present-day events. Replacing it with the local-inertia solver, where the sea connects to the domain boundary, brings the over-prediction to ${\approx}1\times$ (Fig.~\ref{fig:bias}): Bangkok's RP100 coastal extent drops from 3{,}546 to 283~km\textsuperscript{2} (a 12.5$\times$ reduction). We benchmark this corrected layer against coastal-specific references (storm-surge and high-tide inundation footprints), \emph{not} the 2011 compound megaflood, whose fluvial headwaters lie outside the model domain; the 2011 extent is reported only as a loose upper-envelope sanity check. The fix is solver-architecture, not data: Singapore's residual 18$\times$ ratio reflects a near-zero documented baseline, not a model failure.

\begin{figure}[t]
\centering
% \includegraphics[width=\columnwidth]{docs/paper/figures/ieee_fig3_bathtub_bias.png}
...
Code, per-city configuration, and the frozen validation registers (with their pre-reclassification labels) are released openly; for double-blind review an anonymised mirror is provided, and the de-anonymised repository is provided on acceptance, enabling third-party rebuild of every result from free data.
% MERGE [C3]: For non-blind submission restore the URL https://github.com/phangdaniel-debug/ASEAN_flood_model .
% Open item: the Singapore row currently depends on an external "flood-atlas" path in the scoring script and must be migrated into the released repo so the mirror rebuilds all four cities.
...
\bibitem{jpm_seachange} J.P. Morgan, ``Sea Change: Port infrastructure, climate risks and the future of global trade,'' Climate Advisory, 2025. % MERGE [C7]: secondary commercial source; trace headline numbers to primary literature before submission.
...
\bibitem{jpm_intuition} J.P. Morgan, ``Navigating the New Climate Era: Building Intuition for Strategic Decision-Making,'' Climate Intuition, 2025. % MERGE [C7]: secondary commercial source.
...
\bibitem{trinh2017} N. Q. Trinh, T. T. Tran, and H. T. Le, ``Climate change impacts on the Saigon--Dong Nai river system: storm surge and SLR scenarios,'' SIWRR Technical Report, 2017. % MERGE [C4]: grey literature; title/venue unverified --- confirm or replace before submission.
\end{thebibliography}

%% ===========================================================================
%% MERGE CHANGELOG (delete before submission)
%% This file = v2 corrected bibliography + reviewed-PDF body + outstanding fixes.
%%
%% A. ALREADY FIXED IN v2 (carried through):
%%   A1 [1] ADB title corrected.   A2 Lenderink completed (\lenderink2017).
%%   A3 Chaussard initial E.        A4 Wing title/journal (\wing2024).
%%   A5 Hofste -> Aqueduct 3.0.     A6 Renno 2008 added (\renno2008).
%%   A7 Barnes -> Part 3, vol.9, 2021 (\barnes2021).  A8 TopoToolbox removed.
%%
%% B. NOT IN v2 (body was old pre-ports draft) -- brought to reviewed-PDF state:
%%   B1 Title/abstract reframed to ports / forward-looking.
%%   B2 Sec. "Decision Context" + Figs 1-2 added.
%%   B3 Pipeline expanded (exact-determination Gumbel, 250/155 cell, Manning rating,
%%      pluvial steady-state caveat, subsidence separate, CMEMS disclosure).
%%   B4 Table II bathtub->inertial; mitigation delta retraction (-133 -> ~11 km2).
%%   B5 Validation: pre-defined gate, BCa+Fisher, two-bar PASS, Bonferroni,
%%      small-register fragility, Fig.4 benchmark corrected to coastal footprints.
%%   B6 Sec. "From Screening to Strategic Decisions" added.
%%   B7 Conclusion recalibrated (one-of-four PASS; "first such" softened).
%%   B8 New refs added (jpm_seachange, hanson2020, verschuur2023, jpm_intuition,
%%      patel2024); whole list reordered by first citation.
%%
%% C. OUTSTANDING FIXES (not in v2 or PDF) -- applied here:
%%   C1 Table I reframed openness-not-accuracy + FABDEM acknowledgment.
%%   C2 FABDEM named as next-step terrain fix in Limitations.
%%   C3 Reproducibility flags Singapore external-path dependency.
%%   C4 Trinh flagged grey/unverified.
%%   C5 Fig.1 caption labels 2.3/15.9% as schematic normal tail.
%%   C6 Table II footnote: every row uses inertial composite.
%%   C7 JPMorgan refs flagged secondary; trace numbers to primary lit.
%%   C8 Abstract: 28-62% deficit is vs global-synthetic/ERA5-Land, not Fathom.
%%
%% D. COULD NOT VERIFY:
%%   D1 Live GitHub ieee-r10htc.md not fetchable (repo unreachable/anonymised);
%%      merge treats uploaded ieee-r10htc_-_v2.md as current. Re-merge if GitHub differs.
%%   D2 Whether the released repo commits all rasters / rebuilds Singapore end-to-end.
%% ===========================================================================
